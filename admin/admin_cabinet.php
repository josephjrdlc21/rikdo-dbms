<?php 
    ...
    $_SESSION['backpage'] = "admin_cabinet.php";
    ...
    $expire = mysqli_query($conn,"UPDATE research_documents 
    SET instatus='inactive' 
    WHERE date_submitted < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 YEAR) AND status<>'Published' AND date_restore < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 3 YEAR)");
    ...
    if(isset($_GET['status_filter']) && $_GET['status_filter'] != "all"){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents 
        WHERE status='$filter' AND status<>'Published' AND instatus<>'inactive'");
    ...
    elseif(isset($_GET['category']) && $_GET['category'] != "all"){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents,users 
        WHERE users.role='$category' AND research_documents.status<>'Published' AND research_documents.instatus<>'inactive' AND research_documents.submitted_by=users.id");
        $total_records = mysqli_fetch_array($result_count);
        $total_records = $total_records['total_records'];
        $total_no_of_pages = ceil($total_records / $total_records_per_page);
        $second_last = $total_no_of_pages - 1;
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents,users 
        WHERE research_documents.status<>'Published' AND research_documents.instatus<>'inactive' AND research_documents.adviser=users.id");
        $total_records = mysqli_fetch_array($result_count);
        $total_records = $total_records['total_records'];
        $total_no_of_pages = ceil($total_records / $total_records_per_page);
        $second_last = $total_no_of_pages - 1;
    ...
    if(isset($_POST['submit'])){
        if(isset($_POST['file_id'])){
            ...
            $activity = "Moves research documents in archive";
            ...
            echo "<script>swal({title: 'File Deleted', text: 'Check the archive to see deleted files!', icon: 
                'success'}).then(function(){ 
                    window.location.href = 'admin_cabinet.php';
                   }
                );</script>";
        }
    }
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-lg grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 style="overflow-x: auto;" class="text-primary">File Cabinet</h4>
                        ...
                        <div class="row mt-4" style="overflow-x: auto;">
                            <div class="col-lg-12">
                                ...
                                <form action="admin_cabinet.php" method="POST">
