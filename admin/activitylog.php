<?php 
    ...
    $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM activity_log WHERE YEAR(created_at)=$filter");
    $total_records = mysqli_fetch_array($result_count);
    $total_records = $total_records['total_records'];
    $total_no_of_pages = ceil($total_records / $total_records_per_page);
    $second_last = $total_no_of_pages - 1;

    $users_log = "SELECT activity_log.*,users.profile,users.role,users.idnum,users.firstname,users.lastname FROM activity_log,users 
    WHERE activity_log.account=users.id AND YEAR(created_at)=$filter ORDER BY activity_log.id DESC LIMIT $offset, $total_records_per_page";
    ...
    if(isset($_POST['submit'])){
        if(isset($_POST['file_id'])){
            foreach($_POST['file_id'] as $file_id){
                ...
                $sql_delete_log = mysqli_query($conn,"DELETE FROM activity_log WHERE id='$file_id'");
            }
            echo "<script>swal({title: 'User activity log Deleted', text: 'User activity log deleted successfully!', icon: 
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-lg grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 style="overflow-x: auto;" class="text-primary">Activity Log</h4>
