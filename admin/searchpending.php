<?php
    ...
    if(isset($_POST['input'])){
        ...
        if(!empty($input)){
            ...
            $sql_search_file = mysqli_query($conn,"SELECT research_documents.*, users.firstname, users.lastname, degree_strand.ds_code
            FROM research_documents,users,degree_strand
            WHERE research_documents.adviser='$adviser' AND research_documents.status='Pending' AND research_documents.instatus<>'inactive' AND 
            (research_documents.adviser=users.id AND research_documents.degree_strand=degree_strand.id)
            ORDER BY research_documents.date_submitted DESC
            LIMIT 7");
        }
    }
