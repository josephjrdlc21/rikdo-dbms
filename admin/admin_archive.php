<?php 
    ...
    $_SESSION['backpage'] = "admin_archive.php";
    ...
    if(isset($_GET['status_filter']) && $_GET['status_filter'] != "all"){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE adviser='$adviser' AND status='$filter' AND status<>'Published' AND status<>'Pending' 
        AND instatus<>'inactive'");
        ...
        WHERE research_documents.adviser='$adviser' AND research_documents.status='$filter' AND research_documents.status<>'Published' AND research_documents.instatus<>'inactive' 
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE adviser='$adviser' AND status<>'Published' AND instatus<>'inactive' AND status<>'Pending'
        AND research_documents.status<>'Pending'");
        $total_records = mysqli_fetch_array($result_count);
        $total_records = $total_records['total_records'];
        $total_no_of_pages = ceil($total_records / $total_records_per_page);
        $second_last = $total_no_of_pages - 1;
        ...
        $files_list = "SELECT research_documents.*, users.firstname, users.lastname, degree_strand.ds_code
        FROM research_documents,users,degree_strand
        WHERE research_documents.adviser='$adviser' AND research_documents.status<>'Published' AND research_documents.instatus<>'inactive'
        AND research_documents.status<>'Pending' AND (research_documents.adviser=users.id AND research_documents.degree_strand=degree_strand.id)
        ORDER BY research_documents.date_submitted DESC
        LIMIT $offset, $total_records_per_page";
    ...
    if(isset($_POST['submit'])){
        if(isset($_POST['file_id'])){
            foreach($_POST['file_id'] as $file_id){
                ...
                $sql_delete_file = mysqli_query($conn,"UPDATE research_documents SET instatus='inactive' WHERE id='$file_id'");
            }
            ...
            echo "<script>swal({title: 'File Deleted', text: 'Check the archive to see deleted files!', icon: 
                'success'}).then(function(){ 
                    window.location.href = 'admin_archive.php';
                   }
                );</script>";
        }
    }
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-lg grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 style="overflow-x: auto;" class="text-primary">Checked Files</h4>
                                ...
                                <table id="table">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <br><small><?= $row_files_list["authors"];?><br>
