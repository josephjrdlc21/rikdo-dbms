<?php 
    ...
    $_SESSION['backpage_ad'] = "archive.php";
    ...
    if(isset($_GET['status_filter']) && $_GET['status_filter'] != "all"){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE adviser='$adviser' AND status='$filter' AND status<>'Pending'
        ...
        WHERE research_documents.adviser='$adviser' AND research_documents.status='$filter' 
        AND research_documents.status<>'Published' AND research_documents.instatus<>'inactive'
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE adviser='$adviser' AND status<>'Published' AND status<>'Pending' AND instatus<>'inactive'");
        $total_records = mysqli_fetch_array($result_count);
        $total_records = $total_records['total_records'];
        $total_no_of_pages = ceil($total_records / $total_records_per_page);
        $second_last = $total_no_of_pages - 1;
