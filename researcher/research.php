<?php 
    ...
    if(isset($_GET['status_filter']) && $_GET['status_filter'] != "all"){
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE submitted_by='$uploader' AND status='$filter' AND status<>'Published'
        ...
        WHERE research_documents.submitted_by='$uploader' AND research_documents.status='$filter' AND research_documents.status<>'Published' 
        AND research_documents.instatus<>'inactive'
        ...
        $result_count = mysqli_query($conn,"SELECT COUNT(*) As total_records FROM research_documents WHERE submitted_by='$uploader' AND status<>'Published'
        AND instatus<>'inactive'");
        $total_records = mysqli_fetch_array($result_count);
        $total_records = $total_records['total_records'];
        $total_no_of_pages = ceil($total_records / $total_records_per_page);
        ...
        $files_list = "SELECT research_documents.*, users.firstname, users.lastname, degree_strand.ds_code
        FROM research_documents,users,degree_strand
        WHERE research_documents.submitted_by='$uploader' AND research_documents.status<>'Published' AND research_documents.instatus<>'inactive'
        AND (research_documents.adviser=users.id AND research_documents.degree_strand=degree_strand.id)
        ORDER BY research_documents.date_submitted DESC
        LIMIT $offset, $total_records_per_page";
    ...
    <div class="container-fluid py-4">
        <div class="row">
            <div class="col-lg grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        <h4 style="overflow-x: auto;" class="text-primary">Research Files</h4>
